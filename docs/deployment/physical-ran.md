...
| **Playbook guard**                        | Ansible (phase 4 & 5)     | When `physical_ran_enabled` is true, runs `ovs-vsctl br-exists br-ran` on the worker before creating `n2-physical` and before AMF attachment. Prevents pods from failing sandbox setup when the bridge was never created. Bypass: `physical_ran_skip_bridge_check: true` in `group_vars` or `-e physical_ran_skip_bridge_check=true`. |
...
| `failed to find bridge br-ran`          | NAD / AMF reference `br-ran` but OVS never created it | Worker needs RAN NIC + overlay applied; restart `ds-net-setup-worker` on worker. Same root cause as playbook guard failure (table *OVS DaemonSet vs NAD*).                                         |
| Playbook fails before `n2-physical` / AMF RAN | `physical_ran_enabled` true, `br-ran` missing on worker | `vagrant reload worker` with `PHYSICAL_RAN_BRIDGE`, then re-run overlay; or `physical_ran_enabled: false` and enable from dashboard later; or `physical_ran_skip_bridge_check: true` only if deliberate. |
