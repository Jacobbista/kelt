...
INVENTORY
    ...
    # --- Autosource .testbed.env + .testbed.secrets in every shell.
    # Runs on every `vagrant up` and `vagrant reload` (run: "always") so the
    # snippet is in place even when the main provisioner is skipped.
    ...
    #
    # The source line must be the FIRST line of ~/.bashrc. Ubuntu's stock
    # ~/.bashrc returns within its first ten lines when the shell is not
    # interactive, so anything appended at the end never runs for
    # `ssh host "cmd"` or `bash -lc`: exactly the automation and agent paths
    # this is meant to serve. Appending made the variables load only when a
    # human typed at a prompt, which is a silent, hard-to-spot failure.
    # ~/.profile sources ~/.bashrc unconditionally, so line 1 covers login
    # shells too.
    ansible.vm.provision "bashrc-autosource", type: "shell", run: "always", privileged: false,
      inline: <<-'SHELL'
      cat > ~/.testbed-env.sh <<'ENVSH'
# TESTBED_ENV_AUTOSOURCE — generated by Vagrant, do not edit
[ -f /vagrant/.testbed.env ]     && { set -a; . /vagrant/.testbed.env;     set +a; }
[ -f /vagrant/.testbed.secrets ] && { set -a; . /vagrant/.testbed.secrets; set +a; }
ENVSH
      # Drop the appended copy from earlier versions. Both patterns are anchored so
      # they cannot match the sourcing line below, which keeps this idempotent.
      sed -i -e '/^# TESTBED_ENV_AUTOSOURCE$/d' \
             -e '\%^\[ -f /vagrant/\.testbed\.%d' ~/.bashrc
      if ! grep -q 'testbed-env.sh' ~/.bashrc 2>/dev/null; then
        sed -i '1i . "$HOME/.testbed-env.sh"   # TESTBED_ENV_AUTOSOURCE' ~/.bashrc
      fi
    SHELL
